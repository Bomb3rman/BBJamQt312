...
void ContactListener::PostSolve(b2Contact* contact, const b2ContactImpulse* impulse)
{
    ...
    ContactEvent event;
    event.type = ContactEvent::PostSolve;
    event.fixtureA = toBox2DFixture(contact->GetFixtureA());
    event.fixtureB = toBox2DFixture(contact->GetFixtureB());
    event.value = impulse->normalImpulses[0];
    mEvents.append(event);
}
    ...
    if (event->timerId() == mTimer.timerId()) {
        ...
        foreach (const ContactEvent &event, mContactListener->events()) {
            ...
            case ContactEvent::EndContact:
                ...
                break;
            case ContactEvent::PostSolve:
                event.fixtureA->emitPostSolve(event.fixtureB, event.value);
                event.fixtureB->emitPostSolve(event.fixtureA, event.value);
                break;
            default: break;
            }
        }
